...
@click.option(
    ...
    default=None,
    type=click.Path(),
    help="Output trajectory path (default: {scratch_dir}/merged_madeWhole_production.dcd)",
)
@click.option(
    "--segments",
    default=None,
    help="Segment range to process, e.g., '3-7' for production_3 through production_7",
)
@click.option(
    "--strip-solvent",
    is_flag=True,
    help="Exclude water from output (smaller file, but no water analysis possible)",
)
@click.option(
    "--strip-cosolvent",
    is_flag=True,
    help="Exclude co-solvents from output",
)
@click.option(
    "--water-resnames",
...
def postprocess(
    ...
    Examples:
        ...
        polyzymd postprocess -c config.yaml -r 1

        # Process only segments 3-7
        polyzymd postprocess -c config.yaml -r 1 --segments 3-7

        # Strip solvent for smaller visualization file
        polyzymd postprocess -c config.yaml -r 1 --strip-solvent
    ...
    # Parse segment range if provided
    segment_range = None
    if segments:
        try:
            start, end = segments.split("-")
            segment_range = (int(start), int(end))
            click.echo(f"Processing segments {start} through {end}")
        except ValueError:
            click.echo(
                f"Error: Invalid segment range '{segments}'. Use format 'start-end' (e.g., '3-7')",
                err=True,
            )
            sys.exit(1)

    try:
        output_path = make_whole_trajectory(
            config=config,
            replicate=replicate,
            output=output,
        ...
        click.echo(click.style(f"Post-processing failed: {e}", fg="red"), err=True)
        if LOGGER.level == logging.DEBUG:
            import traceback
