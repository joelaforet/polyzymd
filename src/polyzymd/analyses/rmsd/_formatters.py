...
def _format_rmsd_table(result: RMSDComparisonResult) -> str:
    ...
    for run_label in result.run_labels:
        ...
        for condition_label, mean_rmsd, sem_rmsd, rank in ranked_rows:
            ...
            ci_str = format_interval_from_sem(mean_rmsd, sem_rmsd, n_replicates, precision=2)
            lines.append(
                f"{condition.label:<18} {mean_rmsd:<15.2f} {ci_str:<26} {sem_str:<8} {rank:<4}"
            )
...
def _format_rmsd_markdown(result: RMSDComparisonResult) -> str:
    ...
    for run_label in result.run_labels:
        ...
        for condition_label, mean_rmsd, sem_rmsd, rank in ranked_rows:
            ...
            sem_str = format_sem_value(sem_rmsd, n_replicates, precision=2)
            ci_str = format_interval_from_sem(mean_rmsd, sem_rmsd, n_replicates, precision=2)
            lines.append(f"| {condition.label} | {mean_rmsd:.2f} | {ci_str} | {sem_str} | {rank} |")
