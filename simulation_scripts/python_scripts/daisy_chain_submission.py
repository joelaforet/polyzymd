...
"""
python daisy_chain_submission.py -p LipA -o SBMA-EGPMA -n 2 -t 293 -e 0.5 -r 1 -pp NH3_terminal_His_proton_updated.pdb -ps ATRP_SBMA_EGPMA_5-mer -rep 1-2 -chars A B -probs 0.98 0.02 -plen 5 -prefix SBMA-EGPMA -seg 5 --preset blanca-shirts --frames 100
"""
...
INITIAL_JOB_SCRIPT_TEMPLATE ="""#!/bin/bash \n
...
python sim_enz_co-poly_DOCKED_RESTRAINT.py \\
    -p {protein_name} \\
    -o {oligomer_name} \\
    -n {num_oligomers} \\
    -t {temperature} \\
...
def create_initial_job_script(args, replicate_num, segment_index=0, total_segments=10):
    ...
    job_script = INITIAL_JOB_SCRIPT_TEMPLATE.format(
        partition=args.partition,
        job_name=f"s{segment_index}_r{replicate_num}_{args.temperature}K_{args.protein_name}_{args.oligomer_name}-{int(args.probabilities[1]*100)}%",
        job_output=f"s{segment_index}_r{replicate_num}_{args.temperature}K_{args.protein_name}_{args.oligomer_name}-{int(args.probabilities[1]*100)}%.%A_%a.out",
        qos=args.qos,
        ...
        path_to_poly_sdf=args.path_to_poly_sdf,
        replicate_num=replicate_num,
        characters=' '.join(args.characters),
        probabilities=' '.join(map(str, args.probabilities)),
...
def create_continuation_job_script(args, replicate_num, segment_index, total_segments=10, ligand="Resorufin-Butyrate"):
    ...
    working_dir = (
        f"3,3A_RESTRAINT_{args.protein_name}_{ligand}_{args.oligomer_name}-"
        f"{int(args.probabilities[1]*100)}%_{args.num_oligomers}x_{args.temperature}K_"
        f"{args.eq_time}ns-NVT_{args.prod_time}ns-NPT_run{replicate_num}"
    )

    job_script = CONTINUATION_JOB_SCRIPT_TEMPLATE.format(
        partition=args.partition,
        job_name=f"s{segment_index}_r{replicate_num}_{args.temperature}_{args.protein_name}_{args.oligomer_name}-{int(args.probabilities[1]*100)}%",
        job_output=f"s{segment_index}_r{replicate_num}_{args.temperature}_{args.protein_name}_{args.oligomer_name}-{int(args.probabilities[1]*100)}%.%A_%a.out",
        qos=args.qos,
        time_limit=args.time_limit,
        email=args.email,
        account=args.account,
...
def main():
    ...
    if abs(sum(args.probabilities) - 1.0) > 1e-6:
        print("Error: Probabilities must sum to 1.0")
        sys.exit(1)

    # Validate number of characters matches number of probabilities
    if len(args.characters) != len(args.probabilities):
        print("Error: Number of characters must match number of probabilities")
        sys.exit(1)

    total_jobs = len(replicate_list) * args.total_segments
    
    print(f"Preparing {args.total_segments} segment simulation jobs for {args.protein_name}_{args.oligomer_name}")
    ...
    for replicate_num in replicate_list:
        ...
        for segment_index in range(args.total_segments):
            ...
            if segment_index == 0:
                ...
                script_name = f"initial_seg{segment_index}_rep{replicate_num}_{args.protein_name}_RBY_{args.oligomer_name}-{int(args.probabilities[1]*100)}%.sh"
                dependency_job_id = None
            else:
                # Continuation job
                script_content = create_continuation_job_script(args, replicate_num, segment_index, args.total_segments)
                script_name = f"continue_seg{segment_index}_rep{replicate_num}_{args.protein_name}_RBY_{args.oligomer_name}-{int(args.probabilities[1]*100)}%.sh"
