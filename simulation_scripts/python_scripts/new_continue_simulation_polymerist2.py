...
Updated to work with new JSON parameter format.
"""
...
from openmm.unit import Quantity, nanoseconds, kelvin, bar, femtoseconds, picoseconds, atmospheres
...
def add_barostat_if_needed(system, param_dict):
    """Add barostat to system if barostat parameters are specified."""
    thermo_raw = param_dict["__values__"]["thermo_params"]["__values__"]
    
    # Check if barostat parameters exist
    if "barostat_params" in thermo_raw:
        ...
        barostat_freq = barostat_raw.get("update_frequency", 25)
...
def setup_reporters(simulation, working_dir, segment_index, param_dict):
    ...
    # State data reporter - get state data configuration
    if report_state_data:
        state_file = output_dir / f"production_{segment_index}_state_data.csv"
        state_data_config = reporter_raw.get("state_data", {})
        
        simulation.reporters.append(StateDataReporter(
            str(state_file), 
            report_interval,
            step=state_data_config.get("step", True),
            time=state_data_config.get("time", True),
            potentialEnergy=state_data_config.get("potentialEnergy", True),
            kineticEnergy=state_data_config.get("kineticEnergy", True),
            totalEnergy=state_data_config.get("totalEnergy", True),
            temperature=state_data_config.get("temperature", True),
            volume=state_data_config.get("volume", True),
            density=state_data_config.get("density", True),
            speed=state_data_config.get("speed", True),
            progress=state_data_config.get("progress", False),
            remainingTime=state_data_config.get("remainingTime", False),
            elapsedTime=state_data_config.get("elapsedTime", False)
        ))
    
    # Checkpoint reporter
    if report_checkpoint:
        checkpoint_file = output_dir / f"production_{segment_index}_checkpoint.chk"
        # Use num_steps from reporter params if available, otherwise calculate
        checkpoint_interval = reporter_raw.get("num_steps", report_interval)
...
def main():
    ...
    try:
        ...
        param_dict["__values__"]["integ_params"]["__values__"]["total_time"] = {
            "__class__": "Quantity",
            "__values__": {"value": args.segment_time, "unit": "nanosecond"}
        }
