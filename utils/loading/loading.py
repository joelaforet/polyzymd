...
def fix_prot_atoms_names(
    universe: mda.Universe, prot_path: str = "NH3_terminal_His_proton_updated.pdb"
) -> mda.Universe:
...
def write_corrected_protein_topology(
...
def get_representative_frame(universe: mda.Universe, equil_offset: int = 0) -> int:
...
def load_or_transform_universes(
    simulation_id: str,
    thermo_id: str,
    dir_id: str,
    ...
    for temp, num_replicates in temperatures_replicates.items():
        for run in range(1, num_replicates + 1):
            ...
            if (
                os.path.exists(transformed_trajectory)
                and os.path.exists(ref_output_topology)
                and os.path.exists(ref_output_trajectory)
            ):
                ...
                center_prot = transformations.center_in_box(
                    ag=universe.select_atoms(f"protein or resname {ligand_name}"), center="mass"
                )
                workflow = [center_prot]
                universe.trajectory.add_transformations(*workflow)

                # Get representative frame index
                representative_frame_index = get_representative_frame(
                    universe=universe, equil_offset=equilibrium_offset
                )
                ...
                average = align.AverageStructure(
                    universe,
                    universe,
                    select="protein and type CA",
                    ref_frame=representative_frame_index,
                ).run(verbose=True)
                ref = average.results.universe
                print("Starting AlignTraj")
                aligner = align.AlignTraj(
                    universe, ref, select="protein and type CA", in_memory=False
                ).run(verbose=True)
                del aligner
                del average
                ...
                all_atoms.write(
                    ref_output_trajectory, frames=universe.trajectory[[representative_frame_index]]
                )
...
def load_or_transform_daisychain_universes(
    simulation_id: str,
    thermo_id: str,
    dir_id: str,
    temperatures_replicates: dict,
    ...
    for temp, num_replicates in temperatures_replicates.items():
        for run in range(1, num_replicates + 1):
            ...
            legacy_ref_output_trajectory = (
                f"{base_path}/production/ref_prot_centered_trajectory.dcd"
            )
            ...
            if (
                ...
                os.path.exists(legacy_transformed_trajectory)
                and os.path.exists(legacy_ref_output_topology)
                and os.path.exists(legacy_ref_output_trajectory)
            ):
                # Load the transformed universe and reference structure from file
                print("Cached data found!")
                ...
                print(
                    f"Loading {len(all_dcd_files)} DCD files from {len(production_dirs)} production directories"
                )
                ...
                representative_frame_index = (
                    0  # reference is just the first frame, want to refer to equilibrated structure
                )
                average = align.AverageStructure(
                    universe,
                    universe,
