...
| `mda_backend_policy` | mapping | no | `{}` | Deprecated and ignored; see below |
...
## `mda_backend_policy` (deprecated, ignored)

The block chose a backend for MDAnalysis `AnalysisBase.run()`. No analysis
builds an `AnalysisBase` job any more: a plugin reports observables and the
framework calls its `compute()` once per replicate. The block still parses for
one release so an existing campaign file loads, and any non-default value emits
a `UserWarning` and a log line saying it is ignored. It will be rejected in the
next release; delete it from `comparison.yaml`.

| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `backend` | string | `null` | Parsed and ignored |
| `n_workers` | positive int | `null` | Parsed and ignored |
| `n_parts` | positive int | `null` | Parsed and ignored |

To run replicates in parallel, use `polyzymd compare run --workers N`, which is
PolyzyMD-level scheduling and is unaffected by this change.
...
| `donor_acceptor_elements` | list of string | `["N", "O"]` | Elements allowed to act as donors and acceptors; add `"S"` for sulfur. Capitalized and de-duplicated; `"H"` and unknown symbols are rejected |
| `hydrogens_selection` | string | `null` | Override for the hydrogen selection; `element H` by default |
| `composition` | mapping | `null` | Deprecated since v1.3 and ignored; query the event sidecar instead |
| `allow_overlapping_composition` | bool | `false` | Deprecated since v1.3 and ignored, with `composition` |
| `timestep_ps` | float | `null` | Deprecated since v1.3 and ignored; the framework resolves the window |
...
elements remain unavailable raises `SelectionError` instead of
counting every atom as a donor and an acceptor.
Element spellings are taken from the topology, so a topology that writes `CL` is
matched even though the canonical symbol is `Cl`. Set `hydrogens_selection` only
