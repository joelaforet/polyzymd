...
When `reactions` is the bundled `"default"` set, dynamic generation uses the
native linear methacrylate backend and caches charged OpenFF SDF artifacts. The
legacy Polymerist path is retained only for custom `.rxn` workflows. Nonlinear,
branched, and dendrimer polymers are not generated by the dynamic backend; supply
those as charged SDF molecules through `provided_molecules`.
...
| `generation_mode` | string | No | "cached" | `cached`, `dynamic`, or `fragments`; prefer `dynamic` or `fragments` for new generated polymers |
...
| `provided_molecules` | list | No | `[]` | Additive pools of explicit charged SDF molecules |
| `cache_directory` | path | No | ".polymer_cache" | Cache directory for generated and charged polymer artifacts |
| `reactions` | object | Dynamic only | all "default" | ATRP reaction templates; bundled defaults use native methacrylate generation |
