...
5. optional `extract_metrics()` only for non-artifact aggregate outputs, or a custom `compare()`;
...
This simple MDAnalysis artifact path does not implement `extract_metrics()`.
...
handled by `compare_condition_artifacts()` before the legacy/non-artifact
...
Do not add `extract_metrics()` for this all-`ConditionArtifact` path. Instead,
choose stable metric keys in each `ReplicateArtifact.payload["metrics"]`; those
keys and aggregated values are what `compare_condition_artifacts()` uses for the
default comparison.
