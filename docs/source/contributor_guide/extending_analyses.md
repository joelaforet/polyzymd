...
5. condition-artifact comparison for default MDA artifacts, with
   `extract_metrics()` only as a compatibility/fallback helper for non-artifact
   aggregate outputs, or a custom `compare()`;
...
This simple MDAnalysis artifact path does not require `extract_metrics()` as its
primary metric source. Each replicate declares explicit scalar values in
`ReplicateArtifact.payload["metrics"]`; the default aggregator combines those
values into a `ConditionArtifact` with per-metric `mean`, `std`, `sem`, `n`, and
replicate `values`. During comparison, all-`ConditionArtifact` summaries are
handled by `compare_condition_artifacts()` before the compatibility/fallback
`extract_metrics()` branch is reached.
...
Do not rely on `extract_metrics()` as the primary metric path for this
all-`ConditionArtifact` workflow. Instead, choose stable metric keys in each
`ReplicateArtifact.payload["metrics"]`; those keys and aggregated values are
what `compare_condition_artifacts()` uses for the default comparison. The current
scaffold includes `extract_metrics()` as a compatibility/fallback helper for
non-artifact aggregate summaries, not as the main artifact comparison contract.

Implement `extract_metrics(summary)` only when your plugin's aggregate output is
not a `ConditionArtifact`, such as a legacy/custom aggregate model or a plain
dictionary returned by custom `aggregate()` code. In that non-artifact branch,
return `MetricValue` objects with thoughtful `higher_is_better`, unit, and
direction metadata because those fields drive ranking and CLI interpretation.
...
- default `ConditionArtifact` comparison, or `extract_metrics()`/custom comparison for
  non-artifact aggregate outputs;
